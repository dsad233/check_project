...
async def create_leave_category(
    *, branch_id: int, session: AsyncSession, leave_create: LeaveCreate
) -> LeaveCategories:
...
async def find_leave_category_all(
    *, session: AsyncSession, branch_id: int, search: BaseSearchDto = Depends()
) -> list[LeaveCategories]:
    statement = (
        select(LeaveCategories)
        .filter(
            LeaveCategories.deleted_yn == "N", LeaveCategories.branch_id == branch_id
        )
        .offset(search.offset)
        .limit(search.record_size)
    )
