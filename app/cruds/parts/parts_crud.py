...
VALID_AUTO_ANNUAL_LEAVE_GRANTS = ["회계기준 부여", "입사일 기준 부여", "조건별 부여", "수동부여"]
...
async def update_auto_annual_leave_grant(
    *, session: AsyncSession, branch_id: int, part_id: int, auto_annual_leave_grant: str
) -> Parts:
    ...
    if not part:
        ...
        # Enum 값 검증
    if auto_annual_leave_grant not in VALID_AUTO_ANNUAL_LEAVE_GRANTS:
        raise InvalidEnumValueError(f"잘못된 자동 부여 정책 ENUM 입니다: {auto_annual_leave_grant}")

    # 현재 값과 비교하여 변경이 필요한 경우에만 업데이트
    if part.auto_annual_leave_grant != auto_annual_leave_grant:
        await session.execute(
            update(Parts)
            .where(Parts.id == part_id)
            .values(auto_annual_leave_grant=auto_annual_leave_grant)
        )
