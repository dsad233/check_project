...
async def create(
    *, branch_id: int, session: AsyncSession, request: HourWageTemplate
    ...
    hour_wage_template = await find_by_name_and_branch_id(session=session, branch_id=branch_id, name=request.name)
...
async def update(
    *, branch_id: int, hour_wage_template_id: int, session: AsyncSession, request: HourWageTemplate
) -> bool:
