...
from app.models.branches.work_policies_model import (
    BreakTime, WorkPolicies,WorkPoliciesDto, WorkSchedule
)
    ...
    try:
        ...
        if work_policies is None:
            ...
            await session.execute(
                delete(WorkSchedule).where(WorkSchedule.work_policy_id == work_policies.id)
            )
            await session.execute(
        ...
        if auto_overtime_policies is None:
            await auto_overtime_crud.create(session=session, branch_id=branch_id, 
                auto_overtime_policies_create=AutoOvertimePolicies(branch_id=branch_id, **policies_in.auto_overtime_policies.model_dump()))
        else:
            await auto_overtime_crud.update(session=session, branch_id=branch_id, 
                auto_overtime_policies_update=AutoOvertimePolicies(branch_id=branch_id, **policies_in.auto_overtime_policies.model_dump(exclude_unset=True)))
        ...
        if holiday_work_policies is None:
            await holiday_work_crud.create(session=session, branch_id=branch_id, 
                holiday_work_policies_create=HolidayWorkPolicies(branch_id=branch_id, **policies_in.holiday_work_policies.model_dump()))
        else:
            await holiday_work_crud.update(session=session, branch_id=branch_id, 
                holiday_work_policies_update=HolidayWorkPolicies(branch_id=branch_id, **policies_in.holiday_work_policies.model_dump(exclude_unset=True)))

        # OverTimePolicies 업데이트
        overtime_policies = await overtime_crud.find_by_branch_id(session=session, branch_id=branch_id)
        if overtime_policies is None:
            await overtime_crud.create(session=session, branch_id=branch_id, 
                overtime_policies_create=OverTimePolicies(branch_id=branch_id, **policies_in.overtime_policies.model_dump()))
        else:
            await overtime_crud.update(session=session, branch_id=branch_id, 
                overtime_policies_update=OverTimePolicies(branch_id=branch_id, **policies_in.overtime_policies.model_dump(exclude_unset=True)))

        # AllowancePolicies 업데이트
        allowance_policies = await allowance_crud.find_by_branch_id(session=session, branch_id=branch_id)
        if allowance_policies is None:
            await allowance_crud.create(session=session, branch_id=branch_id, 
                allowance_policies_create=AllowancePolicies(branch_id=branch_id,
                    **policies_in.default_allowance_policies.model_dump(),
                    **policies_in.holiday_allowance_policies.model_dump()))
        else:
            await allowance_crud.update(session=session, branch_id=branch_id, 
                allowance_policies_update=AllowancePolicies(branch_id=branch_id,
        ...
        raise HTTPException(status_code=500, detail="근무정책 업데이트에 실패하였습니다.")