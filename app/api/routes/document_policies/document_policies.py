...
async def get_document(branch_id : int, branch_policy_id : int):
    try:
        find_document = await document.execute(select(DocumentPolicies).where(DocumentPolicies.branch_id == branch_id, DocumentPolicies.branch_policy_id ==  branch_policy_id, DocumentPolicies.deleted_yn == "N").offset(0).limit(100))
...
async def get_one_document(branch_id : int, branch_policy_id : int, id : int):
    try:
        find_one_document = await document.execute(select(DocumentPolicies).where(DocumentPolicies.branch_id == branch_id, DocumentPolicies.branch_policy_id ==  branch_policy_id, DocumentPolicies.id == id, DocumentPolicies.deleted_yn == "N"))
        result = find_one_document.scalar_one_or_none()

        if result == None:
            raise HTTPException(status_code=404, detail="문서 정책이 존재하지 않습니다.")
        ...
        print(err)
        raise HTTPException(status_code=500, detail="문서 정책 상세 조회에 에러가 발생하였습니다.") 
...
async def update_document(branch_id : int, branch_policy_id : int, id : int, token=Annotated[Users, Depends(validate_token)]):
    try:
        ...
        print(err)
        raise HTTPException(status_code=500, detail= "문서 정책 업데이트 에러가 발생하였습니다.")
    

# 문서 정책 삭제
@router.delete('/{branch_id}/branch_policies/{branch_policy_id}/document_policies/{id}')
async def delete_document(branch_id : int, branch_policy_id : int, id : int, token=Annotated[Users, Depends(validate_token)]):
    try:
        if token.role != "MSO 최고권한" | token.role != "최고관리자":
            raise HTTPException(status_code=403, detail="접근 권한이 없습니다.")
        ...
        print(err)
        raise HTTPException(status_code=500, detail= "문서 정책 삭제 에러가 발생하였습니다.")


# 문서 정책 소프트 삭제
@router.patch('/{branch_id}/branch_policies/{branch_policy_id}/document_policies/softdelete/{id}')
async def delete_document(branch_id : int, branch_policy_id : int, id : int, token=Annotated[Users, Depends(validate_token)]):
    try:
        if token.role != "MSO 최고권한" | token.role != "최고관리자":
            raise HTTPException(status_code=403, detail="접근 권한이 없습니다.")
