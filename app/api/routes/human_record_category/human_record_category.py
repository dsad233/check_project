...
from app.models.branches.human_record_category_model import (
    HumanRecordCategory,
    HumanRecordCategoryCreate,
    HumanRecordCategoryResponse,
    HumanRecordCategoryUpdate,
)
...
async def get_human_record_category(
    ...
    try:
        ...
        query = select(HumanRecordCategory).where(
            HumanRecordCategory.branch_id == branch_id,
            HumanRecordCategory.deleted_yn == "N",
        )
        result = await db.execute(query)
        human_record_categories = result.scalars().all()

        human_record_category_response = [
            HumanRecordCategoryResponse(
                id=category.id,
...
@router.post("/")
async def create_human_record_category(
    branch_id: int,
    create_dto: HumanRecordCategoryCreate,
    ...
    try:
        ...
        hr_query = select(HumanRecordCategory).where(
            HumanRecordCategory.branch_id == branch_id,
            HumanRecordCategory.category_name == create_dto.category_name,
            HumanRecordCategory.deleted_yn == "N",
        )
        ...
        if hr_category:
            raise HTTPException(
                status_code=400, detail="Human record category already exists"
            )
...
async def update_human_record_category(
    branch_id: int,
    category_id: int,
    update_dto: HumanRecordCategoryUpdate,
    current_user_id: int = Depends(get_current_user_id),
):
    try:
        user_query = select(Users).where(
        ...
        if update_dto.category_name:
            validate_query = select(HumanRecordCategory).where(
                HumanRecordCategory.category_name == update_dto.category_name,
                HumanRecordCategory.branch_id == branch_id,
                HumanRecordCategory.deleted_yn == "N",
                HumanRecordCategory.id != category_id,
            )
        ...
        if not hr_category:
            raise HTTPException(
                status_code=404, detail="Human record category not found"
            )
...
async def delete_human_record_category(
    branch_id: int,
    category_id: int,
    current_user_id: int = Depends(get_current_user_id),
):
