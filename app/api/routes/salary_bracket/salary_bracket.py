...
async def get_salary_bracket(year: int):
    try:
        query = select(SalaryBracket).where(SalaryBracket.year == year)
...
@router.post("/salary_bracket", response_model=SalaryBracketResponse)
async def create_salary_bracket(salary_bracket: SalaryBracketCreate):
    return await SalaryBracket.create_salary_bracket(salary_bracket)
